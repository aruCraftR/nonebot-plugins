...
main_group_notice = on_notice(rule=rule_forbidden_id, permission=permission_main_group)

admin_group_notice = on_notice(permission=permission_admin_group)
...
async def main_group_notice_handler(event: NoticeEvent, bot: Bot):
    ...
    if isinstance(event, GroupRequestEvent):
        for i in shared.plugin_config.mcsm_instances_list:
            try:
                await AsyncMcsmAPI.send_command(i, 'title @a[tag=admin] title "提醒"')
                await AsyncMcsmAPI.send_command(i, 'title @a[tag=admin] subtitle "有新的入群申请待审批"')
            except Exception as e:
